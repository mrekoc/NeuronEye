...
def main():
    ...
    for (img, thresh, file, value) in zip(imgs, threshes, files, values):
        ...
        total, final = sc.hough_circles(img, thresh)
    ...
    for (file, img, thresh, final, value, total, score, recall, precision) in save:
        print("Filename: %s, Detected objects: %d, Actual value: %d, Score: %5.3f, Recall: %5.3f, Precision: %5.3f" % (file, total, value, score, recall, precision))
